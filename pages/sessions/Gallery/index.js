...
export default function Gallery() {
  ...
  const [disce, setDisce] = useState("nil");

  const [ename, setEname] = useState("nil");
  ...
  const handleUploadTc3 = (docs, name) => {
    setIsLoading(true);
    try {
      const storageRef = ref(
        storage,
        `${a.user}/${a.session}/events/${name}.jpg`
      );
      const file = docs;
      const uploadTask = uploadBytesResumable(storageRef, file);
      uploadTask.on(
        ...
        () => {
          ...
          getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
            setImgUrl(downloadURL);
            try {
              const docRef = doc(db, `events`, ename);
              setDoc(docRef, {
                link: downloadURL,
                title: ename,
                disc: disce,
                  ...
                  <div className="text-3xl flex flex-row justify-between w-full ">
                    ...
                    >
                      <span class="relative z-10 block px-5 py-3 overflow-hidden font-medium leading-tight text-gray-800 transition-colors duration-300 ease-out border-2 border-gray-900 rounded-lg group-hover:text-white">
                        <span class="absolute inset-0 w-full h-full px-5 py-3 rounded-lg bg-gray-50"></span>
                        <span class="absolute left-0 w-48 h-48 -ml-2 transition-all duration-300 origin-top-right -rotate-90 -translate-x-full translate-y-12 bg-gray-900 group-hover:-rotate-180 ease"></span>
                        <span class="relative">Add Events</span>
                      </span>
                      <span
                        class="absolute bottom-0 right-0 w-full h-12 -mb-1 -mr-1 transition-all duration-200 ease-linear bg-gray-900 rounded-lg group-hover:mb-0 group-hover:mr-0"
                        data-rounded="rounded-lg"
          ...
          <div class="flex justify-center h-full items-center  antialiased w-full">
            <div class="flex flex-col w-11/12 sm:w-5/6 lg:w-1/2 max-w-2xl mx-auto rounded-lg border border-gray-300 shadow-xl">
              ...
              <div class="flex flex-row items-center justify-between p-5 bg-white border-t border-gray-200 rounded-bl-lg rounded-br-lg">
                ...
                <button
                  onClick={() => {
                    handleUploadTc3(imgtoUpload, ename);
                  }}
                  class="px-4 py-2 text-white font-semibold hover:bg-blue-700 bg-blue-500 rounded"
                >
                  {!isLoading ? (
