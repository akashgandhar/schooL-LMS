...
export default function MapDates() {
  ...
  const mapSubjects = async () => {
    if (!selectedclassName || !d || !subjectName || !selectedExamName) {
      alert("Enter Missing Details");
      console.log(selectedclassName, d, subjectName, selectedExamName);
    } else {
      try {
        const docRef = `users/${a.user}/sessions/${a.session}/exams/${selectedExamName}/classes`;
        await setDoc(doc(db, docRef, selectedclassName), {
          Name: selectedclassName,
        })
          .then(async () => {
            const docRef = `users/${a.user}/sessions/${a.session}/exams/${selectedExamName}/classes/${selectedclassName}/subjects`;
            await updateDoc(doc(db, docRef, subjectName), {
              Date: d,
            });
          })
          .then(() => {
      ...
      <div className="w-screen">
        <div class="bg-gray-100 flex bg-local w-screen">
          <div class="bg-gray-100 mx-auto w-screen h-auto  py-20 px-12 lg:px-24 shadow-xl mb-24">
            <div>
              <h1 className="text-center font-bold text-2xl">Map New Date</h1>
              ...
              <table class="min-w-full border-collapse block md:table">
                <thead class="block md:table-header-group">
                  <tr class="border border-grey-500 md:border-none block md:table-row absolute -top-full md:top-auto -left-full md:left-auto  md:relative ">
                    <th class="bg-gray-600 p-2 w-[40%] text-white font-bold md:border md:border-grey-500 text-left block md:table-cell">
                      Subject Name
                    </th>
                    <th class="bg-gray-600 p-2 w-[40%] text-white font-bold md:border md:border-grey-500 text-left block md:table-cell">
                      Date
